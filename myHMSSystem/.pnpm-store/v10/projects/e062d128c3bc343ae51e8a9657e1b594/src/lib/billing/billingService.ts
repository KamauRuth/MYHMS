...
export async function getBillingSummary(dateFrom?: string, dateTo?: string) {
  ...
  try {
    ...
    const summary = {
      ...
      total_revenue: data?.reduce((sum: number, inv: any) => sum + (inv.total_amount || 0), 0) || 0,
      total_paid: data?.reduce((sum: number, inv: any) => sum + (inv.paid_amount || 0), 0) || 0,
      total_outstanding: data?.reduce((sum: number, inv: any) => sum + (inv.balance || 0), 0) || 0,
      invoices_by_status: {
        unpaid: data?.filter((inv: any) => inv.status === 'unpaid').length || 0,
        partially_paid: data?.filter((inv: any) => inv.status === 'partially_paid').length || 0,
