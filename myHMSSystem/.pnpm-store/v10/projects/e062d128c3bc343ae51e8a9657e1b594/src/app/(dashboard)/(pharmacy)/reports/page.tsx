...
export default function ReportsPage() {
  ...
  const loadReportData = async () => {
    ...
    try {
      ...
      if (dispensing) {
        const revenue = dispensing.reduce((sum: number, item: any) => sum + (item.selling_price_total || 0), 0)
        const profit = dispensing.reduce((sum: number, item: any) => sum + (item.profit || 0), 0)
        const cost = dispensing.reduce((sum: number, item: any) => sum + (item.cost_price_total || 0), 0)
